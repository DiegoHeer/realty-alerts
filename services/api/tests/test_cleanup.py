...
def test_deletes_sold_residence_past_ttl():
    _make(current_status=ListingStatus.SOLD, status_changed_at=PAST_TTL)
...
def test_deletes_sale_pending_residence_past_ttl():
    _make(current_status=ListingStatus.SALE_PENDING, status_changed_at=PAST_TTL)
...
def test_keeps_new_residence_even_if_status_changed_at_is_old():
    residence = _make(current_status=ListingStatus.NEW, status_changed_at=PAST_TTL)
...
def test_keeps_terminal_residence_within_ttl_window():
    residence = _make(current_status=ListingStatus.SOLD, status_changed_at=WITHIN_TTL)
...
def test_keeps_terminal_residence_with_null_status_changed_at():
    residence = _make(current_status=ListingStatus.SOLD, status_changed_at=None)
...
def test_mixed_batch_deletes_only_eligible_rows():
    expired_sold = _make(current_status=ListingStatus.SOLD, status_changed_at=PAST_TTL)
    expired_pending = _make(current_status=ListingStatus.SALE_PENDING, status_changed_at=PAST_TTL)
    fresh_sold = _make(current_status=ListingStatus.SOLD, status_changed_at=WITHIN_TTL)
    new_old = _make(current_status=ListingStatus.NEW, status_changed_at=PAST_TTL)
    null_anchor = _make(current_status=ListingStatus.SOLD, status_changed_at=None)
...
def test_returns_zero_when_nothing_to_delete():
    _make(current_status=ListingStatus.NEW, status_changed_at=PAST_TTL)
