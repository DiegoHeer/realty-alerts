...
```
services/scraper/   Python CDC scraper (BeautifulSoup, Playwright, requests)
...
- Settings split: `realty_api/settings/base.py` holds defaults, `local.py` overrides for dev, `prod.py` overrides for production (DATABASE_URL via dj-database-url, WhiteNoise compression, `SECURE_PROXY_SSL_HEADER`). Select via `DJANGO_SETTINGS_MODULE`.
...
- Auth: internal endpoints are guarded by an `APIKeyHeader` (`X-API-Key`) that compares against `settings.INTERNAL_API_KEY` with `hmac.compare_digest`. Never log or echo the key. Public/mobile auth lands in a follow-up PR.
