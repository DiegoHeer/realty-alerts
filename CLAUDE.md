...
- Auth: internal endpoints are guarded by an `APIKeyHeader` (`X-API-Key`) that compares against `settings.REALTY_API_KEY` with `hmac.compare_digest`. Never log or echo the key. End-user endpoints (e.g. `/v1/me/*`) are guarded by a `HttpBearer` that validates allauth-headless JWT access tokens and sets `request.user`.
...
- Authentication: internal endpoints use `X-API-Key` (constant-time HMAC compare); end-user endpoints use allauth-headless JWT (Bearer token) validated by a `HttpBearer` that sets `request.user`
